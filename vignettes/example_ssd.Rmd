...
```{r model_evts}
init_event_list <- 
  add_tte(arm="noint", evts = c("sick","sicker","death") ,input={ #intervention
    sick <- 0
    sicker <- draw_tte(1,dist="exp", coef1=coef_noint)
    
  }) %>%  add_tte(arm="int", evts = c("sick","sicker","death") ,input={  
    sick <- 0
...
sensitivity_inputs <-add_item(
            ...
            indicators = append(rep(0, length(df_par))[-pos_indicator],1,pos_indicator-1), #vector of indicators, value 0 everywhere except at sens, where it takes value 1
                              )
...
The model is executed as before, just activating the psa_bool option and using the `run_sim_parallel` function to have multiple-cores working at the simulation level, making this implementation faster than the single-core or multiple-core one using `run_sim`. 
