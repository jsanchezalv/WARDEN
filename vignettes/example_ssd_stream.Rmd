---
title: 'Example for a Sick-Sicker-Dead model - Random Number Substreams & Luck Adjustment'
...
We use in this case a luck adjustment as we update the death time to event using the `luck_adj` function. The parameters go from mean 12 to 10, and sd from 3 to 2, so we update the random number and then we redraw the time to event. If instead we needed to reset the time to event (in the case of repeated independent events), then we would set `rnd_b = runif_stream(1,substream_death)` to regenerate the luck.
...
evt_react_list <-
  ...
  add_reactevt(name_evt = "sicker",
               input = {
                 ...
                 #We perform a luck adjustment randomly but being more slightly more likely in the "noint" arm
