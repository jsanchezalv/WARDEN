...
common_all_inputs <- add_item(max_n_cycles = 30) %>%
  add_item( #utilities
    util_v = if(psa_bool){
      setNames(rgamma_mse(util.data$value,util.data$se^2),util.data$name) #in this case I choose a gamma distribution
  ...
  add_item( #costs
    cost_v = if(psa_bool){
      setNames(rgamma_mse(cost.data$value,cost.data$se),cost.data$name) #in this case I choose a gamma distribution
...
```{r model_evts}
init_event_list <- 
  add_tte(arm="noint",evts=c("start","cycle"),input={ #intervention
    start <- 0
    
    cycle <- 1
    
  }) %>%
  add_tte(arm="int",evts=c("start","cycle"),input={ #intervention
    start <- 0
