...
Normally, we would model each of the different events independently, i.e., we would allow the model to naturally choose and express the competitive risks (we would have an event for "nonfatal_CHD", another for "nonfatal_CBVD", etc), but for the sake of replication we will proceed as in the original paper.
...
rcs_du <-  function(du_all, du, t2, b5, b6, a0, a1, a2, a3){
  sum(du_all * pnorm(q = du + b5 * t2 + b6 * (a3 * t2^3 + a2 * t2^2 + a1 * t2 + a0)))
}

disu_sec_f <- function(time,DU_ALL, DU, B5, B6, nl_RCS_ch){
  
  if(time < nl_RCS_ch$knots[1]){
    rcs_du(DU_ALL, DU, time, B5, B6 , 0,0,0,0)
    
  } else if (between(time, nl_RCS_ch$knots[1],nl_RCS_ch$knots[2])) {
    rcs_du(DU_ALL, DU, time, B5, B6 , nl_RCS_ch[[2]][1],nl_RCS_ch[[2]][2],nl_RCS_ch[[2]][3],nl_RCS_ch[[2]][4])
    
    
  } else if (between(time, nl_RCS_ch$knots[2],nl_RCS_ch$knots[3])){
    rcs_du(DU_ALL, DU, time, B5, B6 ,  nl_RCS_ch[[3]][1],nl_RCS_ch[[3]][2],nl_RCS_ch[[3]][3],nl_RCS_ch[[3]][4])
...
rcs_cost_f <- function(time, nl_RCS_ch){
  if(time < nl_RCS_ch$knots[1]){
    ...
    nl_RCS_ch[[2]][1] + nl_RCS_ch[[2]][2] * time + nl_RCS_ch[[2]][3] * time^2 + nl_RCS_ch[[2]][4] * time^3
  } else if (between(time, nl_RCS_ch$knots[2],nl_RCS_ch$knots[3])){
    nl_RCS_ch[[3]][1] + nl_RCS_ch[[3]][2] * time + nl_RCS_ch[[3]][3] * time^2 + nl_RCS_ch[[3]][4] * time^3
  } else {
    nl_RCS_ch[[4]][1] + nl_RCS_ch[[4]][2] * time
  }
}
...
unique_pt_inputs <- add_item(input = {
    ...
    t_firstE_nonfatal_CHD  <- rgompertz(1, rate = exp(lp_first_event_nonfatal_CHD),  shape = first_event_nonfatal_CHD[10])
    t_firstE_nonfatal_CBVD <- rgompertz(1, rate = exp(lp_first_event_nonfatal_CBVD), shape = first_event_nonfatal_CBVD[10])
    t_firstE_CVD_death     <- rgompertz(1, rate = exp(lp_first_event_CVD_death),     shape = first_event_CVD_death[10])
    t_firstE_nonCVD_death  <- rgompertz(1, rate = exp(lp_first_event_nonCVD_death),  shape = first_event_nonCVD_death[10])
...
This is where the biggest differences are between WARDEN and the original model. WARDEN simulates each patient individually, while the original paper takes advantage of vectorization to speed things up, at the cost of 1) transparency, and 2) lack of adaptability to other model structures. This is particularly impactful in this model as both costs and utilities have a restricted cubic spline approach, which means we need to use `adj_val()` to calculate how they would change over time and integrate that value. This is computationally heavy, and in our case we choose to update these values monthly, as the imprecision is expected to be minimal.
...
evt_react_list <-
               ...
               input = {
                 ...
                 cost_2 <- adj_val(
                      ...
                      (.time-curtime) * coef_cost_1 +
                        coef_cost_2 * rcs_cost_f((.time-curtime), nl_RCS[["nl_RCS_pre"]][[sex + 1]]),
                      discount = drc
                 )
                 ...
                 adj_factor <- adj_val(
                      ...
                      next_event()$time,
                      by = 1/12,
                      u_norms$utility[.time + bs_age < (u_norms$age_max)][1],
                      discount = drq
                 )
  ...
  add_reactevt(name_evt = "event_1",
              ...
              if(status_1>2){ #if death stop
                ...
                t_postSurv_CHD  <- rgompertz(1, rate = exp(lp_post_CHD), shape = post_CHD[5])
                t_postSurv_CBVD <- rgompertz(1, rate = exp(lp_post_CBVD), shape = post_CBVD[5])
                
                v_second_times <- c(t_postSurv_CHD,t_postSurv_CBVD) 
                ...
                linpred_cost <- ifelse(status_2>4,0,linpred_cost[status_1])
                coef_cost_1  <- ifelse(status_2>4,0,coef_c1_post[1,status_1])
                coef_cost_2  <- ifelse(status_2>4,0,coef_c1_post[2,status_1])
                cost_2 <- adj_val(
                     curtime,
                     next_event()$time,
                     by = 1/12,
                     (.time-curtime) * coef_cost_1 +
                       coef_cost_2 * rcs_cost_f((.time-curtime), nl_RCS[[status_1+1]][[sex + 1]]),
                     discount = drc
                )
                ...
                adj_factor <- adj_val(
                      ...
                      by = 1/12,
                      u_norms$utility[.time + bs_age < (u_norms$age_max)][1] -
                        disu_sec_f(.time - curtime,v_du[-6], lp_disusecond, b5, b6, nl_RCS[[status_1+1]][[sex + 1]]),
                      discount = drq
                )
...
```{r model_run}
results <- run_sim(  
  npats=50,                               # number of patients to be simulated
  ...
  arm_list = c("int", "comp"),             # intervention list
...
summary_results_det(results[[1]][[1]], arm ="int") #print first simulation
...
The running times with the original WARDEN approach are comparable to those of simmer shown by the paper, even if simmer uses a complex C++ implementation in the back-end. By using the reworked approach, WARDEN is able to cut running time by half. In my computer simmer took between 220 and 300 seconds to run 100,000 simulations. With WARDEN, the "one to one" approach took around 200 seconds. The reworked approach took 100 seconds.
...
ggplot(merged_data %>% mutate(linetype = ifelse(grepl("simmer",type),"simmer","WARDEN")) 
       , aes(x=pat_id,y=ICER, group = type, col = linetype))+
...
ggplot(merged_data %>% mutate(linetype = ifelse(grepl("simmer",type),"simmer","WARDEN")),
       aes(x=pat_id,y=inc_costs, group = type, col = linetype))+
    geom_line(linewidth=1.1) +
  theme_bw()  +
  ylim(92500,94500)+
    theme(legend.position="bottom")
...
ggplot(merged_data %>% mutate(linetype = ifelse(grepl("simmer",type),"simmer","WARDEN")),
       aes(x=pat_id,y=inc_qalys, group = type, col = linetype))+
  geom_line(linewidth=1.1) +
  theme_bw() +
  ylim(1.8,2.1)+
    theme(legend.position="bottom")
