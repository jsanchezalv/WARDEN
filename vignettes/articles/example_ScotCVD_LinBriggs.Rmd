...
unique_pt_inputs <- add_item(input = {
    ...
    t_firstE_nonfatal_CHD  <- qgompertz(rnd_v_1[1], rate = exp(lp_first_event_nonfatal_CHD),  shape = first_event_nonfatal_CHD[10])
    t_firstE_nonfatal_CBVD <- qgompertz(rnd_v_1[2], rate = exp(lp_first_event_nonfatal_CBVD), shape = first_event_nonfatal_CBVD[10])
    t_firstE_CVD_death     <- qgompertz(rnd_v_1[3], rate = exp(lp_first_event_CVD_death),     shape = first_event_CVD_death[10])
    t_firstE_nonCVD_death  <- qgompertz(rnd_v_1[4], rate = exp(lp_first_event_nonCVD_death),  shape = first_event_nonCVD_death[10])
...
evt_react_list <-
  ...
  add_reactevt(name_evt = "event_1",
              ...
              if(status_1>2){ #if death stop
                ...
                t_postSurv_CHD  <- qgompertz(rnd_v_2[1], rate = exp(lp_post_CHD), shape = post_CHD[5])
                t_postSurv_CBVD <- qgompertz(rnd_v_2[2], rate = exp(lp_post_CBVD), shape = post_CBVD[5])
                
                v_second_times <- c(t_postSurv_CHD,t_postSurv_CBVD, time_horizon - curtime) 
