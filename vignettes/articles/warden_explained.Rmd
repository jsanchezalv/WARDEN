...
The way WARDEN processes events through e.g., `modify_item` is by looking at what inputs are currently available from the relevant input list, evaluating the passed list (e.g., `modify_item_seq(list(a = 1, b = a + 1))`) and adding the resulting objects (`a` and `b`) to the parent environment as well as to the relevant input list for storage. With the latest update of WARDEN, mobjects not included in `modify_item` will be stored in the relevant input list and they will be available for evaluation, e.g.,
