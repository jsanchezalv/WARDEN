...
init_event_list <- 
  add_tte(arm=c("noint","int"), evts = c("sick","sicker","death") ,input={ #intervention
    sick <- 0
    sicker <- qexp(rnd_stream_a$draw_n(),exp(coef_noint + ifelse(arm=="int",log(HR_int),0)))
               ...
               input = {
                 ...
                 if((common_luck + ifelse(arm=="noint", 0.1,0) ) >0.7){
                    rnd_stream_b$random_n <- luck_adj(prevsurv = 1 - pnorm(q=curtime,12,3),
                                                      ...
                                                      luck = rnd_stream_b$draw_n(),
                                                      condq = FALSE)
                    death <- max(0.0000001,qnorm(rnd_stream_b$random_n, mean=10, sd=2))
                 }
