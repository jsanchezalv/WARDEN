...
#Common within an arm
common_arm_inputs <- add_item2(input ={
...
If the patient fails to block the bed, then the time to death is accelerated by a factor of 0.8. Note that if we are using the resource constrained approach, we need to use `modify_event2` instead of `modify_event` (and similarly, we would have `new_event2` instead of `new_event`. In this case, `modify_event2` is automatically sequential (so it works similarly to `modify_event_seq`). Note that because resources are shared, we can 1) know who the next patient in the queue is, and 2) create a new event for that specific patient, even if the current patient being evaluated is another.
  ...
  add_reactevt(name_evt = "sicker",
               input = {
                 ...
                 if(!success_blocking_bed){
                   ...
                   modify_event2(c(death = max(curtime,get_event("death") * 0.8)))
                   had_to_queue <- 1
                 }else{
...
It can be seen that with constrained = FALSE, the results are equivalent to the SSD standard vignette. Furthermore, if the model is run with constrainted = TRUE but in such a way that the constraint does not bind (e.g., with 1000+ beds) then the results are also equivalent.
```{r model_run_unc}
#Logic is: per patient, per intervention, per event, react to that event.
results <- run_sim(  
  npats=1000,                               # number of patients to be simulated
