...
Log files are created both in RDICE and DICE, but the details included are different, and log files are not created on error in RDICE, as R has a debug method which would allow the user to navigate live through the code at the moment of error. A log file will be created if debug option is activated.
