...
- Rcpp event based handler has been created using queues for high
  efficiency. In the new system, a unique event per patient is accepted
  in the queue, so small changes may be needed in codes where multiple
  equally named events were set to accommodate the new system. This is
  to ensure best practices and clarity regarding which events are
  modified, in the queue, etc.
...
- Rcpp based discrete resources with their own queuing system have been
  implemented (only interacted through R, working similar to R6
...
- Shared inputs have been created to be used in constrained DES (works
  similar to R6 objects). This can be created by using shared_input().
...
- Rcpp versions of key functions have been implemented for speed
  improvements: disc_cycle_v, disc_ongoing_v, disc_instant_v, qcond\_\*,
  qtimecov, luck_adj
...
- Now the model can be run without specifying utilities, costs or other
  outputs (only LYs will be accounted for)
...
- Per cycle outcomes now correctly work, and a maximum number of cycles
  argument has been implemented.

## WARDEN 1.2.1

CRAN release: 2025-06-19

- Discounting now correctly allocates to drq or drc depending on output.
  Other inputs use drc.
...
- Major update: now the engine uses environments instead of lists, which
  allows user to remove “modify_item” and “modify_item_seq” from their
  code, improving running speed by 20-40%
- Secondary changes to accommodate this update applied throughout
  (extract reactions, debug mode).
- Debug mode now uses abstract syntax tree to capture assignments, which
  can be limited in the presence of dynamic code assignments.
...
- runif_stream function has been removed due to violation of CRAN policy
  of global environment modification. It is suggested that the user
...
- Now debug and continue_on_error will work on all stages, not only for
  simulations
...
- Added possibility of continuing to the next simulation on error (if it
  occurs at the patient/arm level, not at the statics/structural loading
  level)
- Debug mode now exports log even if the simulation stops due to error.
  If combined with continue on error, it will continue to export log
  with the timestamp
...
- Sensitivity-level and simulaton-level seeds have been moved outside
  the input loading loop as this caused correlation between inputs
  loaded at those stages.
